...
const AdminProduct = () => {
  ...
  const renderAction = () => {
    return (
      <div>
        <DeleteOutlined style={{ color: '#000', fontSize: '30px', cursor: 'pointer' }} onClick={() => setIsModalOpenDelete(true)}/>
        <EditOutlined style={{ color: '#000', fontSize: '30px', cursor: 'pointer' }} onClick={handleDetailsProduct} />
      </div>
    )
  }
  ...
  const handleDeleteProduct = () => {
    ...
    const onFinish = () => {
      ...
      mutation.mutate(params, {
        onSettled: () => {
        queryProduct.refetch()
        console.log('Thông tin sản phẩm mới:', params); // Thêm log ở đây
  ...
  const handleChangeSelect = (value) => {
      ...
      <div>
        <WrapperHeader>Quản lý sản phẩm</WrapperHeader>
          <div style={{ marginTop: '10px' }}> 
            <Button style={{ height: '100%', width: '200px', borderRadius: '6px', border: 'solid' }} onClick={() => setIsModalOpen(true)}><div style={{ fontSize: '20px', display: 'inline'}}>Thêm sản phẩm </div><PlusOutlined style={{ fontSize: '20px', height: '100%' }}/></Button>
          </div>
    ...
    <ModalComponent title="Xóa sản phẩm" open={isModalOpenDelete} onCancel={handleCancelDelete} onOk={handleDeleteProduct}>
      <Loading isLoading={isLoadingDeleted}>
        <div>Bạn có muốn xóa sản phẩm</div>
      </Loading>
    </ModalComponent>
      </div>
    )
